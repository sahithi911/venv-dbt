...
logger = AdapterLogger("Ingres")


@dataclass
class IngresCredentials(Credentials):
    host: str
    user: str
    port: Port
    password: str  # on ingres the password is mandatory
...
class IngresConnectionManager(SQLConnectionManager):
    TYPE = "ingres"
    ...
    def exception_handler(self, sql):
        try:
            ...
            logger.debug("Ingres error: {}".format(str(e)))
    ...
    def open(cls, connection):
        ...
        kwargs = {}
        # we don't want to pass 0 along to connect() as ingres will try to
        ...
        if search_path is not None and search_path != "":
            # see https://ingresql.org/docs/9.5/libpq-connect.html
