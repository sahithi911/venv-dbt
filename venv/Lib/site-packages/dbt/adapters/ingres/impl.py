...
from dbt.adapters.ingres import IngresConnectionManager
from dbt.adapters.ingres.column import IngresColumn
from dbt.adapters.ingres import IngresRelation
...
GET_RELATIONS_MACRO_NAME = "ingres_get_relations"


@dataclass
class IngresIndexConfig(dbtClassMixin):
    columns: List[str]
    unique: bool = False
    ...
    @classmethod
    def parse(cls, raw_index) -> Optional["IngresIndexConfig"]:
...
@dataclass
class IngresConfig(AdapterConfig):
    unlogged: Optional[bool] = None
    indexes: Optional[List[IngresIndexConfig]] = None


class IngresAdapter(SQLAdapter):
    Relation = IngresRelation
    ConnectionManager = IngresConnectionManager
    Column = IngresColumn

    AdapterSpecificConfigs = IngresConfig
    ...
    @available
    def parse_index(self, raw_index: Any) -> Optional[IngresIndexConfig]:
        return IngresIndexConfig.parse(raw_index)
    ...
    def _get_catalog_schemas(self, manifest):
        # Ingres only allow one database (the main one)
